...
class SkillLevel:
    ...
    @validate_call
    def progress_percentage(self, elo: int = Field(ge=MIN_ELO), /) -> float | None:
        if self.is_highest_level:
            warnings.warn(
    ...
    def get_level(
        ...
        *,
        elo: int | None = Field(None, ge=MIN_ELO),
    ) -> Self | None:
    ...
    @classmethod
    @validate_call
    def get_all_levels(cls, game_id: GameID, /) -> list[Self]:
